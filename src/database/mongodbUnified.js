/**
 * MongoDB Database Handler - Unified Model
 * 
 * Updated to use ConversationUnified model with embedded messages
 */
...
async function createIndexes() {
  try {
    // Indexes for ConversationUnified collection
    await db.collection('conversationunifieds').createIndex({ 'contact.phoneNumberNormalized': 1, userId: 1 }, { unique: true });
    await db.collection('conversationunifieds').createIndex({ userId: 1, lastMessageAt: -1 });
    await db.collection('conversationunifieds').createIndex({ userId: 1, status: 1, lastMessageAt: -1 });
    await db.collection('conversationunifieds').createIndex({ 'messages.whatsappMessageId': 1 }, { sparse: true });
...
exports.saveMessageToConversation = async (conversationId, messageData) => {
  try {
    ...
    const existingConv = await database.collection('conversationunifieds').findOne({
      _id: conversationId,
    ...
    const result = await database.collection('conversationunifieds').updateOne(
      { _id: conversationId },
      {
        $push: {
          messages: messageDoc
 ...
 * Using ConversationUnified model
 */
exports.findOrCreateConversation = async ({ phoneNumber, name, userId, lastMessageText, lastMessageTimestamp }) => {
  try {
    ...
    let conversation = await database.collection('conversationunifieds').findOne({
    ...
    const result = await database.collection('conversationunifieds').insertOne({
      contact: {
    ...
    conversation = await database.collection('conversationunifieds').findOne({
...
exports.getConversation = async (conversationId) => {
  const database = await exports.connectToDatabase();
  return await database.collection('conversationunifieds').findOne({
...
exports.getMessages = async (conversationId, limit = 50) => {
  const database = await exports.connectToDatabase();
  const conversation = await database.collection('conversationunifieds').findOne(
    { _id: new ObjectId(conversationId) },
